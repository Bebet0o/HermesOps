...
if [[ -n "$NODE_BIN" ]]; then
  ...
  echo "HERMESOPS_2U_CONSOLE_NODE_SYNTAX_PASS binary=$NODE_BIN"
else
  echo "HERMESOPS_2U_CONSOLE_NODE_SYNTAX_SKIPPED reason=node_unavailable"
fi

cd "$REPO"
python3 -m unittest -v tests.test_console_objective_lifecycle

echo "HERMESOPS_2U_CONSOLE_OBJECTIVE_LIFECYCLE_PASS"
